...
function onInput(e) {
   ...
   e.preventDefault();
    ...
    fetchV(value).then((contry) => { if (contry.length === 1) createListCountry(contry, singleContry); return contry; })
        .then((contry) => {
            if (contry.length < 10 && contry.length > 1)
                createListCountry(contry, contries)
            return contry
        })
        .then(contry => {
            if (contry.length > 10) alert({
